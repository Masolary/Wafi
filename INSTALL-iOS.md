...
2. Download its **NovelCool-Safari-iOS-unsigned** artifact and unzip it to obtain `NovelCool-Safari-iOS-unsigned.ipa`.
3. Use your existing legitimate iOS sideloading/signing method. The IPA is **unsigned**, so simply opening it on your iPhone/iPad will not install it. You do not need a Mac if your signing method supports Windows.
...
3. Open the novel/chapter in Safari and **reload**. Tap **Clean reading** on the green toolbar if you want the text-only reading view.
